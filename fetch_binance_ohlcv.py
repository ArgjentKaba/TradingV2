...
def fetch_all_ohlcv(ex, symbol: str, since_ms: int, timeframe: str) -> pd.DataFrame:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                chunk = ex.fetch_ohlcv(
                    symbol, timeframe=timeframe, since=next_since, limit=REQUEST_LIMIT
                )
                break
            ...
            raise RuntimeError(
                f"Failed to fetch after {MAX_RETRIES} retries for {symbol}"
            )
    ...
    df = pd.DataFrame(
        all_rows, columns=["time_ms", "open", "high", "low", "close", "volume"]
    )
...
def main():
    ...
    for sym in tqdm(SYMBOLS, desc="Downloading"):
        ...
        mins = len(merged)
        print(
            f"[OK] {sym} -> {venue} | rows={mins}, range={first_dt} .. {last_dt} | saved: {out_path}"
        )
